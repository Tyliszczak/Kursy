...
test('dodanie kierowcy nie uruchamia triala, pierwsza aktywacja uruchamia go tylko raz',()=>{
  const f=fixture(),c=f.company(),sessionToken=f.admin(c.id),identity={sessionToken,deviceId:'admin-1',fingerprint:'fp'};f.api.activateAdminDevice_(identity);const d=f.api.addDriver_({...identity,name:'Jan',phone:'500600700'}).driver;assert.equal(f.api.publicLicense_(c.id).trialStartedAt,null);const token=f.api.createDriverActivation_({...identity,driverId:d.id}).activationToken;const first=f.api.activateDriverDevice_({activationToken:token,deviceId:'driver-1',fingerprint:'fp1'});assert.equal(first.license.status,'trial_active');const started=first.license.trialStartedAt;f.api.updateOne_('Admins',x=>x.companyId===c.id,{email:'zmiana@example.com'});f.api.updateOne_('Drivers',x=>x.id===d.id,{phone:'+48999999999'});const resumed=f.api.driverStatus_({driverSessionToken:first.driverSession.token,deviceId:'driver-1',fingerprint:'fp1'});assert.equal(resumed.mayUse,true);assert.equal(f.api.publicLicense_(c.id).trialStartedAt,started);
});
test('sesja kierowcy odnawia się bez nowego linku, rotuje tokeny i pozostaje przypisana do urządzenia',()=>{
  const f=fixture(),c=f.company(),sessionToken=f.admin(c.id),identity={sessionToken,deviceId:'admin',fingerprint:'fp'};f.api.activateAdminDevice_(identity);const driver=f.api.addDriver_({...identity,name:'Jan',phone:'500600700'}).driver,activationToken=f.api.createDriverActivation_({...identity,driverId:driver.id}).activationToken,active=f.api.activateDriverDevice_({activationToken,deviceId:'phone-1',fingerprint:'phone-fp'}),old=active.driverSession;f.api.updateOne_('DriverSessions',x=>x.driverId===driver.id,{expiresAt:'2000-01-01T00:00:00.000Z'});const refreshed=f.api.refreshDriverSession_({refreshToken:old.refreshToken,deviceId:'phone-1',fingerprint:'phone-fp'}).driverSession;assert.notEqual(refreshed.token,old.token);assert.notEqual(refreshed.refreshToken,old.refreshToken);assert.equal(f.api.driverStatus_({driverSessionToken:refreshed.token,deviceId:'phone-1',fingerprint:'phone-fp'}).mayUse,true);code('DRIVER_REFRESH_EXPIRED',()=>f.api.refreshDriverSession_({refreshToken:old.refreshToken,deviceId:'phone-1',fingerprint:'phone-fp'}));code('DRIVER_DEVICE_MISMATCH',()=>f.api.refreshDriverSession_({refreshToken:refreshed.refreshToken,deviceId:'phone-1',fingerprint:'other'}));
});
