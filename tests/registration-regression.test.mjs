...
import {createRegistration,confirmEmail,confirmPhone,isValidPolishNip,normalizeEmail,normalizePhone} from '../js/registration-model.js';
...
test('email and phone verification do not start trial',()=>{const r=createRegistration(valid);confirmEmail(r);confirmPhone(r);assert.equal(r.status,'trial_pending');assert.equal(r.trialStartedAt,null)});
test('phone cannot be confirmed before email',()=>{const r=createRegistration(valid);assert.throws(()=>confirmPhone(r),/e-mail/)});
