const SHEETS={COMPANIES:'Companies',ADMINS:'Admins',VERIFICATIONS:'Verifications',SESSIONS:'Sessions',LICENSES:'Licenses',HISTORY:'LicenseHistory',PAYMENTS:'Payments'};
const HEADERS={
...
function doGet(){return json_({ok:true,service:'kursy-license-api',version:'1.0.0'})}
function doPost(e){
  try{
    const body=JSON.parse(e.postData&&e.postData.contents||'{}');
    rateLimit_(body.action||'unknown');
    const actions={registerCompany:registerCompany_,verifyEmail:verifyEmail_,verifyPhone:verifyPhone_,login:login_,createCheckout:createCheckout_,confirmCheckout:confirmCheckout_,licenseStatus:licenseStatus_};
...
function createCheckout_(p){
  ...
  if(!secret||!priceId)throw apiError_('PAYMENTS_NOT_CONFIGURED','Płatności nie są jeszcze skonfigurowane.');
