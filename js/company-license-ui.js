...
function deviceRows(devices,role){
  ...
  return devices.map(d=>{const owner=role==='driver'?`<small>${t('devices.assignedTo',{name:esc(driverName(d.userId))})}</small>`:'';return `<div class="licenseRow"><span><strong>${esc(d.deviceId.slice(0,20))}â€¦</strong>${owner}<small>${t('devices.lastSeen',{date:formatDate(d.lastSeenAt||d.activatedAt)})}</small></span><button class="btn dangerText" data-release-device="${role}:${esc(d.deviceId)}">${t('common.release')}</button></div>`}).join('');
}
function render(){
  ...
  root.innerHTML=`<h2>${t('license.title')}</h2><p class="muted">${t('license.intro')}</p><div class="licenseSummary"><strong>${esc(company.name)}</strong><span class="licenseStatus"${trialPending?` title="${esc(statusHint)}" tabindex="0" role="button" aria-label="${esc(statusText)}. ${esc(statusHint)}" data-trial-hint="1"`:''}>${esc(statusText)}</span><span>${t('license.trial',{start:formatDate(l.trialStartedAt),end:formatDate(l.trialEndsAt)})}</span>${l.monthlyPrice!==null?`<span>${esc(l.monthlyPrice)} ${esc(l.currency)} / miesiÄ…c</span>`:''}</div><div class="licenseGrid"><section><h3>${t('drivers.title',{used:company.drivers.length,limit:l.limits.drivers})}</h3><form id="driverForm" class="ownerFields"><label>${t('drivers.name')}<input name="name" required></label><label>${t('drivers.phone')}<input name="phone" type="tel" required></label><label>${t('drivers.email')}<input name="email" type="email"></label><button class="btn primary" type="submit">${t('drivers.add')}</button></form>${warning}<div class="licenseList">${drivers}</div></section><section><h3>${t('devices.admin',{used:company.adminDevices.length,limit:l.limits.adminDevices})}</h3>${deviceRows(company.adminDevices,'admin')}<h3>${t('devices.driver',{used:company.driverDevices.length,limit:l.limits.driverDevices})}</h3>${deviceRows(company.driverDevices,'driver')}</section></div>`;
}
function fail(error){root.innerHTML=`<div class="notice"><strong>${esc(error.message||error)}</strong><p class="muted">Dane nie zostaĹ‚y zapisane lokalnie. SprawdĹş poĹ‚Ä…czenie z centralnym backendem.</p></div>`}
...
document.addEventListener('click',event=>{
  const hint=event.target.closest('[data-trial-hint]');if(hint){alert(hint.getAttribute('title'));return}
  ...
  const release=event.target.closest('[data-release-device]');if(release){const [role,deviceId]=release.dataset.releaseDevice.split(':');if(!confirm('ZwolniÄ‡ to urzÄ…dzenie?'))return;busy(release,async()=>{const result=await licenseCloudApi.releaseDevice(role,deviceId);company=result.company;render()})}
});
