...
function render(){
  ...
  root.innerHTML=`<h2>${t('license.title')}</h2><p class="muted">${t('license.intro')}</p><div class="licenseSummary"><strong>${esc(company.name)}</strong><span class="licenseStatus"${trialPending?` title="${esc(statusHint)}"`:''}>${esc(statusText)}</span><span>${t('license.trial',{start:formatDate(l.trialStartedAt),end:formatDate(l.trialEndsAt)})}</span>${l.monthlyPrice!==null?`<span>${esc(l.monthlyPrice)} ${esc(l.currency)} / miesiąc</span>`:''}</div><div class="licenseGrid"><section><h3>${t('drivers.title',{used:company.drivers.length,limit:l.limits.drivers})}</h3><form id="driverForm" class="ownerFields"><label>${t('drivers.name')}<input name="name" required></label><label>${t('drivers.phone')}<input name="phone" type="tel" required></label><label>${t('drivers.email')}<input name="email" type="email"></label><button class="btn primary" type="submit">${t('drivers.add')}</button></form>${warning}<div class="licenseList">${drivers}</div></section><section><h3>${t('devices.admin',{used:company.adminDevices.length,limit:l.limits.adminDevices})}</h3>${deviceRows(company.adminDevices,'admin')}<h3>${t('devices.driver',{used:company.driverDevices.length,limit:l.limits.driverDevices})}</h3>${deviceRows(company.driverDevices,'driver')}</section></div>`;
}
