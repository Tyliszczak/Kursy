...
async function load(){if(!token&&!loadDriverSession()){render();return}try{status=await licenseCloudApi.driverStatus(token,getDeviceIdentity());render()}catch(error){root.innerHTML=`<div class="notice">${esc(error.code==='INVALID_ACTIVATION'?t('driver.badLink'):translateMessage(error.message))}</div>`}}
