...
import {licenseCloudApi,loadDriverSession} from './license-cloud-api.js';
...
function openDriverApp(){localStorage.removeItem(TOKEN_KEY);history.replaceState({},'',location.pathname);location.assign(appUrl())}
function render(){
...
async function load(){if(!token&&!loadDriverSession()){render();return}try{status=await licenseCloudApi.driverStatus(token,getDeviceIdentity());if(queryToken)history.replaceState({},'',location.pathname);render()}catch(error){root.innerHTML=`<div class="notice">${esc(error.code==='INVALID_ACTIVATION'?t('driver.badLink'):translateMessage(error.message))}</div>`}}
