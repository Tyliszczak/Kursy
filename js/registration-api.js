...
async function request(action,payload={}){
  ...
  if(!response.ok||!data.ok) throw new Error(data.message||'Operacja nie powiodĹ‚a siÄ™.');
  return data;
}
...
function localPreview(action,payload){
  ...
  if(action==='login'){
    ...
    if(!found) return Promise.reject(new Error('Nie znaleziono konta w tej przeglÄ…darce.'));
  ...
  if(action==='createCheckout') return Promise.reject(new Error('PĹ‚atnoĹ›Ä‡ zostanie uruchomiona po skonfigurowaniu Stripe w backendzie.'));
  return Promise.reject(new Error('NieobsĹ‚ugiwana operacja.'));
}
