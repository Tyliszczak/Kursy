(()=>{
  ...
  style.textContent=`
    .editorWindowBar{display:grid;grid-template-columns:minmax(0,1fr) auto auto;align-items:center;gap:8px;margin:0 0 14px;padding:8px 0;background:#fff;box-shadow:0 1px 0 rgba(16,24,40,.08)}
    #routeEditor>.editorWindowBar{position:sticky;top:74px;z-index:4;padding:10px 0}
    ...
    #stopEditorModal .editorWindowBar{position:sticky;top:0;z-index:2;margin:0 -20px 14px;padding:10px 20px;border-radius:18px 18px 0 0}
  ...
  function ensureStopBar(){const m=document.getElementById('stopEditorModal');const open=!!m&&!m.hidden;document.body.classList.toggle('stopEditorOpen',open);if(!open)return;const card=m.querySelector('.modalCard');if(!card)return;let b=card.querySelector(':scope > .editorWindowBar');if(!b){const cancel=()=>document.getElementById('stopEditorCancel')?.click();const save=()=>document.getElementById('stopEditorSave')?.click();b=makeBar(`Trasa: ${clean(document.getElementById('routeName')?.value)}`,cancel,save);card.prepend(b)}const t=b.querySelector('.editorWindowTitle'),v=`Trasa: ${clean(document.getElementById('routeName')?.value)}`;if(t.textContent!==v)t.textContent=v}
  function refresh(){ensureRouteBar();ensureStopBar()}
  document.addEventListener('click',e=>{if(e.target.closest('[data-edit-route],[data-edit-stop],#addRouteBtn,#stopEditorCancel,#stopEditorSave'))setTimeout(refresh,0)},true);
