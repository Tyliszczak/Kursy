...
function updateCourseHeader(serviceId,value){
  document.querySelectorAll('.routeTable').forEach(table=>{
    ...
    const text=`Na ${value}`;if(th&&th.textContent!==text)th.textContent=text;
  });
}
...
document.addEventListener('click',e=>{
  ...
  const removeService=e.target.closest('.removeService');if(removeService){syncAllCards();return}
},true);
