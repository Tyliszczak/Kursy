...
function ensureUi(){
  if(!document.querySelector('[data-view="punctuality"]')){
    const nav=document.querySelector('.nav');
    const adminButton=nav?.querySelector('[data-view="admin"]');
    const button=document.createElement('button');button.dataset.view='punctuality';button.innerHTML='📊 Analiza punktualności';
    if(adminButton)nav.insertBefore(button,adminButton);else nav?.appendChild(button);
  }
  if(!$('#view-punctuality')){
    const content=document.querySelector('.content');
    const section=document.createElement('section');section.id='view-punctuality';section.className='view';section.hidden=true;
    section.innerHTML='<div class="panel"><div class="sectionTop"><div><h2>Analiza punktualności</h2><p class="muted">System szuka tylko powtarzalnych i istotnych odchyleń. Pojedyncze korki i minutowe różnice nie powodują propozycji zmian.</p></div><button class="btn" id="refreshPunctuality">Odśwież analizę</button></div><div id="punctualityResults"><div class="notice info">Otwórz analizę, aby sprawdzić zebrane przejazdy.</div></div></div>';
...
export async function refreshPunctualityPanel(){
  ensureUi();const root=$('#punctualityResults');if(!root)return;
  root.innerHTML='<div class="notice info">Analizuję przejazdy…</div>';
  try{const response=await fetchEvents();const events=Array.isArray(response.events)?response.events:Array.isArray(response.data)?response.data:[];const recommendations=analyzePunctuality(events);if(!recommendations.length)return renderEmpty(root);root.innerHTML=summarizeRouteRecommendations(recommendations).map(routeHtml).join('')}catch(error){renderError(root,error)}
}

ensureUi();
document.addEventListener('click',event=>{const nav=event.target.closest('[data-view="punctuality"]');if(nav)setTimeout(refreshPunctualityPanel,0);if(event.target.closest('#refreshPunctuality'))refreshPunctualityPanel()});
