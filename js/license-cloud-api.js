...
export const licenseCloudApi={
  ...
  driverStatus:async(activationToken,identity,presence={})=>{const session=await ensureDriverSession(identity,true);return requestApi('driverStatus',{activationToken:session?'':activationToken,driverSessionToken:session?.token||'',...identity,...presence})},
  driverHeartbeat:async(identity,presence={})=>{const session=await ensureDriverSession(identity);if(!session)return null;return requestApi('driverStatus',{activationToken:'',driverSessionToken:session.token||'',...identity,...presence,heartbeat:true})},
