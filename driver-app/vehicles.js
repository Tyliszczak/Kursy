(()=>{
  const context=window.KURSY_DRIVER_CONTEXT||{};
  const API_URL=context.dataApiUrl||'';
  const scope=[context.companyId,context.driverId].filter(Boolean).join('.')||'unassigned';
  const CACHE_KEY=`trasy2.vehicles.v2.${scope}`;
  const SELECTED_KEY=`trasy2.selectedVehicle.v2.${scope}`;
  let vehicles=[];
